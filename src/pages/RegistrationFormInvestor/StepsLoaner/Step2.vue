<template>
  <ContentWrapper small>
    <Heading center text="Добавьте фото профиля" />
...
import { updateInvestor } from "../../../api/investor";
...
export default {
  ...
  mounted() {
    if (localStorage.imageInvestor) {
      this.image = localStorage.getItem("imageInvestor");
      this.preview = localStorage.getItem("imageInvestor");
    }
  },
  methods: {
    updateInvestor(data) {
      updateInvestor(data).then((res) => {
    ...
    previewImage: function (event) {
      ...
      if (input.files) {
        ...
        reader.onload = (e) => {
          ...
          localStorage.setItem("imageInvestor", this.preview);
        };
    ...
    reset: function () {
      this.image = null;
      this.preview = null;
      localStorage.removeItem("imageInvestor");
    },

    async submitForm() {
      let updateData = new FormData();
      updateData.append("photo", this.image);
      this.updateInvestor(updateData);
    },
  },
};
</script>
