<template>
  <ContentWrapper small>
    <Heading center text="Добавьте сканы/фото паспорта" />
...
import { uploadInvestorFiles } from "../../../api/investor";
...
export default {
  ...
  created() {
    let returnFiles = JSON.parse(localStorage.getItem("filesInvestor"));
    this.file = returnFiles;
  },
  mounted() {
    if (localStorage.filesInvestor) {
  ...
  methods: {
    ...
    uploadInvestorFiles(data) {
      uploadInvestorFiles(data).then((res) => {
        console.log(res, "res");
        this.$emit("btnNext");
      });
    },
    handleFilesUpload() {
      ...
      console.log(uploadedFiles[0].name, "uploaded");
    ...
    sendFiles() {
      ...
      localStorage.setItem("filesInvestor", JSON.stringify(this.localStorage));
      for (let i = 0; i < this.$refs.files.files.length; i++) {
        let fileInvestor = this.$refs.files.files[i];
        data.append("file", fileInvestor);
      }
      this.uploadInvestorFiles(data);
    },
  },
};
</script>
