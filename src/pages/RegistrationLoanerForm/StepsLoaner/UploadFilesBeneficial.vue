...
import { uploadBeneficialPassportFiles } from "@/api/borrower";

export default {
  name: "UploadFilesBeneficial",
  components: {
    ContentWrapper,
    Heading,
    Button,
  ...
  methods: {
    ...
    uploadBeneficialFiles(data) {
      uploadBeneficialPassportFiles(data).then((res) => {
        console.log("res", res);
        this.$emit("btnNext");
      });
    },
    ...
    sendFiles() {
      ...
      localStorage.setItem("passportBeneficialFiles", JSON.stringify(this.localStorage));
      for (let i = 0; i < this.$refs.files.files.length; i++) {
        let passportBorFiles = this.$refs.files.files[i];
        data.append("file", passportBorFiles);
      }
      data.append("beneficiary_id", localStorage.getItem('id_create_beneficial'))
      console.log('data send', data)
      this.uploadBeneficialFiles(data);
    },
  },
};
</script>
